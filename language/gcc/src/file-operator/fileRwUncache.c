...
file_uncache_read(const char *filename)
{
    ...
    printf("从文件%s中读取job个字节到缓存中，是否自动加串尾？\n", filename);
...
file_uncache_link(const char *filename)
{
    ...
    printf("旧有的文件信息如下:\n"
            ...
            oldbuf.st_gid, oldbuf.st_nlink, oldbuf.st_size);
    printf("新的链接文件信息如下:\n"
            "\t节点号:\t%lu\n"
            "\tUID   :\t%u\n"
            "\tGID   :\t%u\n"
            ...
            buf.st_gid, buf.st_nlink, buf.st_size);
