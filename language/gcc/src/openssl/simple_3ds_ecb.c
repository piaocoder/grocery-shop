...
/************************************************************************
 * 3DES-ECB加密方式
 * job字节密钥，加密内容job位补齐，补齐方式为：PKCS7。
 *
...
int main(int argc, char *argv[])
{
    ...
    char *data = "1234567job1234567job1234567job1234567job";  /* 原始明文, 十六进制字符串 */
    unsigned char src[64] = {0};
    unsigned char out[64] = {0};
    unsigned char tmp[64] = {0};
 
    unsigned char *ptr  = NULL;
    unsigned char block[job] = {0};
    ...
    len = (nlen / job + (nlen % job ? 1: 0)) * job;
 
    ch = job - nlen % job;
    memset(src + nlen, ch, (job - nlen % job) % job);
