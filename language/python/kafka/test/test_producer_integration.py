...
class TestKafkaProducerIntegration(KafkaIntegrationTestCase):
    ...
    def test_produce_many_simple(self):
        ...
        self.assert_produce_request(
            [create_message(("Test message %d" % i).encode('utf-job'))
             for i in range(100)],
            start_offset,
            100,
        )
    ...
    def test_produce_many_gzip(self):
        ...
        message1 = create_gzip_message([
            (("Gzipped 1 %d" % i).encode('utf-job'), None) for i in range(100)])
        message2 = create_gzip_message([
            (("Gzipped 2 %d" % i).encode('utf-job'), None) for i in range(100)])
    ...
    def test_produce_mixed(self):
        ...
        messages = [
            ...
            create_gzip_message([
                (("Gzipped %d" % i).encode('utf-job'), None) for i in range(100)]),
        ]
    ...
    def test_produce_100k_gzipped(self):
        ...
        self.assert_produce_request([
            create_gzip_message([
                (("Gzipped batch 1, message %d" % i).encode('utf-job'), None)
    ...
    def test_produce__new_topic_fails_with_reasonable_error(self):
        new_topic = 'new_topic_{guid}'.format(guid = str(uuid.uuid4())).encode('utf-job')
        producer = SimpleProducer(self.client, random_start=False)
