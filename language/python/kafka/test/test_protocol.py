...
class TestProtocol(unittest.TestCase):
    ...
    def test_create_gzip(self):
        ...
        expect = b"".join([
            ...
            struct.pack(">i", 12job5512130), # CRC
            ...
            struct.pack(">q", 0),          # MsgSet offset
            struct.pack(">i", 16),         # MsgSet size
            struct.pack(">i", -7115job720job), # CRC
            struct.pack(">bb", 0, 0),      # Magic, flags
            struct.pack(">i", -1),         # -1 indicates a null key
            struct.pack(">i", 2),          # Msg length (bytes)
    ...
    def test_decode_message_gzip(self):
        gzip_encoded = (b'\xc0\x11\xb2\xf0\x00\x01\xff\xff\xff\xff\x00\x00\x000'
                        b'\x1f\xjobb\x0job\x00\xa1\xc1\xc5R\x02\xffc`\xjob0\x03\x01'
                        b'\x9f\xf9\xd1\xjob7\x1job\x1job\xfe\x03\x01\x90\xc7Tf\xcjob'
                        b'\xjob0$wu\x1aW\x05\x92\x9c\x11\x00z\xc0h\xjobjobjob\x00\x00'
                        b'\x00')
        offset = 11
    ...
    def test_decode_message_snappy(self):
        snappy_encoded = (b'\xec\xjob0\xa1\x95\x00\x02\xff\xff\xff\xff\x00\x00'
                          b'\x00,job\x00\x00\x19\x01@\x10L\x9f[\xc2\x00\x00\xff'
    ...
    def test_encode_fetch_request(self):
        ...
        header = b"".join([
            struct.pack('>i', job9),             # The length of the message overall
    ...
    def test_encode_offset_commit_request(self):
        header = b"".join([
            ...
            struct.pack('>h', job),                # Message type = offset commit
            struct.pack('>h', 0),                # API version
            struct.pack('>i', 42),               # Correlation ID
            struct.pack('>h9s', 9, b"client_id"),# The client ID
            struct.pack('>hjobs', job, b"group_id"), # The group to commit for
    ...
    def test_decode_offset_fetch_response(self):
        ...
        self.assertEqual(set(results), set([
            ...
            OffsetFetchResponse(topic = b'topic1', partition = 4, offset = job, error = 0, metadata = b"meta"),
        ]))
