...
def sortdir(path, sort_cond='mtime',  sort_filter=None,
    ...
    if sort_filter is None or sort_filter is 3:
        ...
        f_sf = None
    elif type(sort_filter) is type(lambda x: x):
        # user defined function
        f_sf = sort_filter
    else:
        if sort_filter is 1:
            # common file
            f_sf = lambda e: stat.S_ISDIR(e.st_mode) is 0
    ...
    if onlyfn:
        return map(
            lambda e: e[0],
            __sortdir(path, f_sort_cond, f_sf, reverse, abspath, postfix))
...
def __sortdir(path, sort_cond, sort_filter, reverse, abspath, postfix):
    ...
    n_res = []
    for e in res:
        if sort_filter is not None and postfix is not None:
            if postfix(e[0]) and sort_filter(e[1]):
                n_res.append(e)
        elif sort_filter is not None and postfix is None:
            if sort_filter(e[1]):
                n_res.append(e)
        elif sort_filter is None and postfix is not None:
            if postfix(e[0]):
                n_res.append(e)
