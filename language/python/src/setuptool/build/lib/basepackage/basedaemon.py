...
class Daemon(object):
    ...
    """
    def __init__(self, pidfile, globalLog, stdin='/dev/null', stdout='/dev/null', stderr='/dev/null'):
        self.stdin = stdin
        self.stdout = stdout
        self.stderr = stderr
    ...
    def daemonize(self):
        ...
        try:
            ...
            fd.write('%s\n'%(str(os.getpid())))
            fd.close()
            os.chmod(self.pidfile, 0777)
        except IOError,msg:
            self._globalLog.getError().log(self._globalLog.ERROR, "%s:open pidfile failed."%msg)
            sys.exit(1)
    
    def sigInt(self, signum, frame):
        '''SIGINT'''
        self._globalLog.getError().log(self._globalLog.ERROR, "Receive SIGINT, eixt process")
        sys.exit(0)
    ...
    def stop(self):
        ...
        try:
            ...
            if err.find("No such process") > 0:
                ...
                self._globalLog.getError().log(self._globalLog.ERROR, message%(pid, self.pidfile))
            else:
    ...
    def stopforce(self, cmdfile):
        ...
        pid = str(os.getpid())
        commandstr = "ps -ef|grep python|grep -w '%s'|grep -v grep|awk '{print $2}'|grep -v %s|xargs kill -9"%(cmdfile,pid)
